...
namespace ldjam_58
{
    public class SoulMovementController : MonoBehaviour
    {
        ...
        public void UpdateMovement(SoulMovementStyle movementStyle)
        {
            ...
            float speed = moveSpeed * ((movementStyle == SoulMovementStyle.Sprinter && _sprinting) ? sprintMultiplier : 1f);
            transform.position += Vector3.right * (speed * Time.deltaTime);

            switch (movementStyle)
            {
                case SoulMovementStyle.Runner: break;
                case SoulMovementStyle.Sprinter: UpdateSprinter(); break;
                case SoulMovementStyle.Hopper: UpdateHopper(); break;
                case SoulMovementStyle.SineWave: UpdateSine(); break;
                case SoulMovementStyle.ZigZag: UpdateZigZag(); break;
                case SoulMovementStyle.Ducker: UpdateDucker(); break;
            }
        }
        ...
        void UpdateSprinter()
        {
            switch (_sprinting)
            {
                case false when _t >= _nextSprintToggle:
                    DoSprint();
                    break;
                case true when _t >= _nextSprintToggle:
                    StopSprint();   
                    break;
            }
        }
